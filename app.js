...
const MAX_IMG_BYTES = 1.5 * 1024 * 1024; // tope objetivo por imagen: 1.5 MB
const QUALITY_STEPS = [0.82, 0.7, 0.6, 0.5, 0.4]; // si sigue pesando mucho, baja calidad (nunca la resolucion)
...
async function handleSave() {
  ...
  try {
    ...
    state.errorMsg = "No se pudo guardar. Intenta de nuevo.";
    render();
  }
}
    ...
    <div class="form-box">
      ...
      <div class="field-row">
        ...
        ${(f.imagenes || []).length < MAX_IMAGENES ? `
          <input type="file" id="f-imagenes" accept="image/*" multiple style="margin-top:8px;" />
          <p class="hint" style="margin-top:4px;">Puedes agregar hasta ${MAX_IMAGENES - (f.imagenes || []).length} imagen(es) mas.</p>
        ` : `<p class="hint" style="margin-top:8px;">Ya alcanzaste el maximo de ${MAX_IMAGENES} imagenes. Quita alguna para poder agregar otra.</p>`}
...
function attachHandlers() {
  ...
  if (imgInput) imgInput.onchange = (e) => {
    const cupo = MAX_IMAGENES - (state.form.imagenes || []).length;
    const files = Array.from(e.target.files).slice(0, cupo);
